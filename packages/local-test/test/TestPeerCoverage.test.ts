...
describe("TestPeer coverage", () => {
  it.effect("pairs both transport directions and maps send failures to ReplicaError", () =>
    Effect.scoped(Effect.gen(function*() {
      ...
      const fromA = Option.getOrThrow(yield* Stream.runHead(b.receive))
      assert.strictEqual(fromA._tag, "Durable")
      if (fromA._tag === "Durable") assert.deepStrictEqual(fromA.delivery.message, bytes(1))
      yield* b.send(bytes(2))
      const fromB = Option.getOrThrow(yield* Stream.runHead(a.receive))
      assert.strictEqual(fromB._tag, "Durable")
      if (fromB._tag === "Durable") assert.deepStrictEqual(fromB.delivery.message, bytes(2))
      yield* a.close
