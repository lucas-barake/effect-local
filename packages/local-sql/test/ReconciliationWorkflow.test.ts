...
describe("reconciliation workflow", () => {
  it.effect("resumes the version 2 workflow for a migrated legacy membership", () =>
    Effect.gen(function*() {
      ...
      yield* sql`UPDATE effect_local_client_spaces
        SET membership_incarnation = ${Identity.legacyMembershipIncarnation}
      ...
      yield* Layer.build(
        ReconciliationWorkflow.layerRegistration({ definition: Domain.definition, spaceId, clientId }).pipe(
          Layer.provide(Layer.succeed(LocalStore.Store, local)),
          Layer.provide(
            Layer.succeed(
              Reconciler.Reconciliation,
              Context.get(reconciliationContext, Reconciler.Reconciliation)
            )
          ),
      ...
      yield* ReconciliationWorkflow.makeLegacyV2(payload).execute(payload).pipe(
