...
const formatTime = (millis: number) => DateTime.formatIso(DateTime.makeUnsafe(millis))
const StringArrayJson = Schema.fromJsonString(Schema.Array(Schema.String))
const HostileDocumentLineage = Schema.declare<Identity.DocumentLineage>(
  (value): value is Identity.DocumentLineage => typeof value === "string"
)
...
describe("PeerSync", () => {
  ...
  ) => InternalAutomerge.stage<E>(document, Automerge.getActorId(document), change)
  ...
  const sourcePeer = (
    ...
    initial: Automerge.Doc<InternalAutomerge.Root<{ readonly title: string; readonly labels: ReadonlyArray<string> }>>
  ) =>
    Effect.gen(function*() {
      ...
      const drain = Effect.gen(function*() {
        ...
        yield* Effect.die("Source sync did not settle")
      })
      return {
        current: () => document,
  ...
  it.effect("scopes relay deduplication by sender without colliding with direct receipts and prunes usage", () =>
    Effect.gen(function*() {
      ...
      const receiptExpiresAt = formatTime(
  ...
  it.effect("settles a detached reply fragment after its relay receipt expires", () =>
    Effect.gen(function*() {
      ...
      const reply = received.reply
      if (reply === null) return yield* Effect.die("Expected a reply")
      const outbound = yield* sync.enqueue(session, reply)
      if (outbound === null) return yield* Effect.die("Expected a pending reply fragment")
  ...
  it.effect("persists inbound application and exact retransmission replies", () =>
    Effect.gen(function*() {
      ...
      const created = yield* store.create(Task, documentId, { title: "one", labels: [] })
      const remote = changeDocument(Automerge.clone(created.automerge, { actor: "1".repeat(32) }), (draft) => {
        const value = draft
        value.title = "remote"
  ...
  it.effect("enqueues a bounded reply sequence for a stale peer", () =>
    Effect.gen(function*() {
      ...
      const drain = Effect.gen(function*() {
        ...
        yield* Effect.die("Remote sync did not settle")
      })
      for (let index = 0; index < 8; index++) {
        remote = changeDocument(remote, (draft) => {
          const value = draft
  ...
  it.effect("splits generated replies by encoded byte size through the production sync path", () =>
    Effect.gen(function*() {
      ...
      const reply = received.reply
      if (reply === null) return yield* Effect.die("Expected a byte bounded reply")
      yield* sync.enqueue(staleSession, reply)
      const pending = yield* sync.pending(staleSession)
  ...
  it.effect("rejects unmeasured and corrupt history counters without durable effects", () =>
    Effect.gen(function*() {
      const counterCases: ReadonlyArray<{
  ...
  it.effect("persists the wire declared writer provenance for an inbound change, not the receiver's own", () =>
    Effect.gen(function*() {
      ...
      const session = yield* sync.open(yield* Identity.makePeerId)
      const created = yield* store.create(Task, documentId, { title: "one", labels: [] })
      const remote = changeDocument(Automerge.clone(created.automerge, { actor: "7".repeat(32) }), (draft) => {
        draft.title = "from another build"
      })
      ...
      const first = yield* sync.receive(Task, documentId, session, {
        ...
        writerProvenance: provenanceFor(firstGenerated[1]).map((entry) => {
          if (entry.changeHash === remoteChangeHash) {
            return { changeHash: entry.changeHash, writerSchemaVersion, writerDefinitionHash }
          }
          return entry
        })
      })
  ...
  it.effect("rebinds a durable reply to each local connection epoch", () =>
    Effect.gen(function*() {
      ...
      const created = yield* store.create(Task, documentId, { title: "one", labels: [] })
      const remote = changeDocument(Automerge.clone(created.automerge, { actor: "9".repeat(32) }), (draft) => {
        draft.title = "remote"
      })
      const message = Automerge.generateSyncMessage(remote, Automerge.initSyncState())[1]!
  ...
  it.effect("rejects connection sequence reuse for another document", () =>
    Effect.gen(function*() {
      ...
      const remote = changeDocument(Automerge.clone(first.automerge, { actor: "8".repeat(32) }), (draft) => {
        draft.title = "remote"
      })
      const message = Automerge.generateSyncMessage(remote, Automerge.initSyncState())[1]!
  ...
  it.effect("rejects altered sequence retransmissions without durable mutation", () =>
    Effect.gen(function*() {
      ...
      const session = yield* sync.open(yield* Identity.makePeerId)
      const created = yield* store.create(Task, documentId, { title: "one", labels: [] })
      const remote = changeDocument(Automerge.clone(created.automerge, { actor: "2".repeat(32) }), (draft) => {
        draft.title = "two"
      })
      const message = Automerge.generateSyncMessage(remote, Automerge.initSyncState())[1]!
      yield* sync.receive(Task, documentId, session, {
  ...
  it.effect("keeps accepting resolved sync receipts past the peer receipt quota", () =>
    Effect.gen(function*() {
      ...
      const session = yield* sync.open(yield* Identity.makePeerId)
      const created = yield* store.create(Task, documentId, { title: "one", labels: [] })
      const remote = changeDocument(Automerge.clone(created.automerge, { actor: "6".repeat(32) }), (draft) => {
        draft.title = "two"
      })
      const message = Automerge.generateSyncMessage(remote, Automerge.initSyncState())[1]!
    ...
    Effect.gen(function*() {
      ...
      for (const sequence of [0, 1]) {
        yield* sql`INSERT INTO effect_local_peer_receipts (
          ...
          ${`pending-${sequence}`}, NULL, NULL, ${message}, '[]', ${encodeStringArray(["a".repeat(64)])}, 0,
          ${formatTime(0)}
        )`
      }
      const received = yield* sync.receive(Task, documentId, session, {
    ...
    Effect.gen(function*() {
      ...
      const makePending = (actor: string, sequenceBase: number) =>
        Effect.gen(function*() {
          ...
          remote = changeDocument(remote, (draft) => {
            draft.title = `${actor}-first`
          })
          const dependency = Automerge.generateSyncMessage(remote, remoteState)
          remoteState = dependency[0]
          assert.isNotNull(dependency[1])
          remote = changeDocument(remote, (draft) => {
            draft.labels.push(`${actor}-second`)
          })
          const dependent = Automerge.generateSyncMessage(remote, remoteState)
          assert.isNotNull(dependent[1])
          yield* sync.receive(Task, documentId, session, {
  ...
  it.effect("rejects pending receipts above the replica quota while isolating incarnations", () =>
    Effect.gen(function*() {
      ...
      yield* sql`INSERT INTO effect_local_peer_receipts (
        ...
        'other-incarnation', NULL, NULL, ${new Uint8Array([1])}, '[]', ${encodeStringArray(["f".repeat(64)])}, 0,
        ${formatTime(0)}
      )`
  ...
  it.effect("serializes one document across sessions without blocking independent documents", () =>
    Effect.gen(function*() {
      ...
      const firstRemote = changeDocument(
        Automerge.clone(firstCreated.automerge, { actor: "a".repeat(32) }),
        (draft) => {
          draft.title = "first remote"
        }
      )
      const secondRemote = changeDocument(
        Automerge.clone(secondCreated.automerge, { actor: "b".repeat(32) }),
        (draft) => {
          draft.title = "second remote"
        }
      )
      ...
      const blockingStore = new Proxy(store, {
        get(target, property, receiver) {
          ...
          const load: typeof store.load = (document, requestedId) => {
            if (requestedId === firstDocumentId) {
              return Deferred.succeed(firstStarted, undefined).pipe(
                Effect.andThen(Deferred.await(releaseFirst)),
                Effect.andThen(store.load(document, requestedId))
              )
            }
  ...
  it.effect("rejects operation-heavy messages without persisting the rejected transition", () =>
    Effect.gen(function*() {
      ...
      const session = yield* sync.open(yield* Identity.makePeerId)
      const created = yield* store.create(Task, documentId, { title: "one", labels: [] })
      let remote = changeDocument(Automerge.clone(created.automerge, { actor: "4".repeat(32) }), (draft) => {
        const value = draft
        value.title = "complex"
  ...
  it.effect("quarantines expired pending sync data without deleting applied changes", () =>
    Effect.gen(function*() {
      ...
      const session = yield* sync.open(yield* Identity.makePeerId)
      const created = yield* store.create(Task, documentId, { title: "one", labels: [] })
      const remote = changeDocument(Automerge.clone(created.automerge, { actor: "5".repeat(32) }), (draft) => {
        draft.title = "fresh"
      })
      const message = Automerge.generateSyncMessage(remote, Automerge.initSyncState())[1]!
      const messageHash = yield* Canonical.digest(message)
      ...
      const remaining = yield* sql<{
        ...
        (SELECT COUNT(*) FROM effect_local_peer_receipts
          WHERE receive_sequence = 0 AND accepted_at = ${formatTime(0)}) AS expiredReceipts,
              ...
              Effect.tap((durable) =>
                Effect.gen(function*() {
                  ...
                  const staged = yield* store.stage(durable, (draft) => {
                    if (
                      typeof draft === "object" && draft !== null && "labels" in draft && Array.isArray(draft.labels)
                    ) {
                      draft.labels.push(`conflict-${conflicts}`)
                    }
                  })
  ...
  it.effect("rejects mismatched durable pending change metadata before replay", () =>
    Effect.gen(function*() {
      ...
      const session = yield* sync.open(yield* Identity.makePeerId)
      const created = yield* store.create(Task, documentId, { title: "one", labels: [] })
      const corrupted = changeDocument(
        Automerge.clone(created.automerge, { actor: "d".repeat(32) }),
        (draft) => {
          draft.title = "corrupted"
        }
      )
      ...
      yield* Effect.gen(function*() {
        ...
        yield* sql`INSERT INTO effect_local_changes (
          ...
          ${encodeStringArray(decoded.deps)}, ${changeBytes[0]},
  ...
  it.effect("rejects hostile lineage values at the direct receive boundary", () =>
    Effect.gen(function*() {
      ...
      for (
        ...
        const exit = yield* Effect.exit(sync.receive(
          Task,
          documentId,
          session,
          {
            remoteConnectionEpoch: session.connectionEpoch,
            receiveSequence: 0,
            lineage: Schema.decodeUnknownSync(HostileDocumentLineage)(lineage),
            message,
            writerProvenance: provenanceFor(message, Task.version, definition.hash)
          }
        ))
            ...
            store.load(document, requestedId).pipe(
              Effect.tap(() => {
                if (requestedId === documentId) {
                  return Deferred.succeed(loadStarted, undefined).pipe(
                    Effect.andThen(Deferred.await(releaseLoad))
                  )
                }
                return Effect.void
  ...
  const checkpointSourceLayer = (
    ...
    const bindings: { readonly projections: []; checkpointAuthority?: CheckpointAuthority.Implementation } = {
      projections: []
    }
    if (authority !== undefined) bindings.checkpointAuthority = authority
    const services = SqlReplica.servicesLayerWithBindings(definition, bindings).pipe(
      Layer.provideMerge([
        SqliteClient.layer({ filename: ":memory:", disableWAL: true }),
        NodeCrypto.layer,
    ...
    Effect.gen(function*() {
      ...
      const current = yield* checkpointOf(
        ...
        changeDocument(
          ...
          (draft) => {
            draft.title = "same-lineage-current"
          }
        )
      )
      ...
      const drain = Effect.fnUntraced(function*() {
        for (let round = 0; round < 32; round++) {
          while (pending.length > 0) {
            ...
            if (packet.outbound.checkpointTransfer !== undefined) {
              Object.assign(input, { checkpointTransfer: packet.outbound.checkpointTransfer })
            }
            const received = yield* packet.to.sync.receive(Task, documentId, packet.to.session, input)
        ...
        yield* Effect.die("post-checkpoint peer sync did not reach quiescence")
      })
  ...
  it.effect("rejects generation when the retained lineage chain exceeds the wire bound", () =>
    Effect.gen(function*() {
      ...
      for (const [index, lineage] of lineages.entries()) {
        let priorLineage = Identity.genesisLineage
        if (index > 0) {
          priorLineage = Option.getOrThrow(Option.fromUndefinedOr(lineages[index - 1]))
        }
        yield* sql`INSERT INTO effect_local_lineage_transitions (
          ...
          ${documentId}, ${priorLineage},
          ${checkpointHash}, ${encodedHeads}, ${snapshot}, ${lineage}, ${checkpointHash}, ${encodedHeads},
          ${Task.version}, ${definition.hash}, ${checkpointToken}, ${formatTime(0)}
        )`
      }
    ...
    Effect.gen(function*() {
      ...
      const current = yield* checkpointOf(
        ...
        changeDocument(
          ...
          (draft) => {
            draft.title = "after-rewrite-mutation"
          }
        )
      )
    ...
    Effect.gen(function*() {
      ...
      const current = yield* checkpointOf(
        ...
        changeDocument(
          ...
          (draft) => {
            draft.title = "post-rewrite-mutation"
          }
        )
      )
