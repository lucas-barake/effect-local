...
describe("ReplicaGate", () => {
  ...
  it.effect("blocks exclusive restore while shared work is active and advances the epoch", () =>
    Effect.gen(function*() {
      ...
      const exclusive = yield* Effect.forkChild(Effect.scoped(Effect.gen(function*() {
        const permit = yield* gate.exclusive
        yield* Deferred.succeed(exclusiveAcquired, permit)
      })))
      ...
      assert.strictEqual(permit.incarnation, 1)
      assert.strictEqual(permit.writerGeneration, 2)
  ...
  it.effect("finishes the epoch transition when interrupted after commit", () =>
    Effect.scoped(Effect.gen(function*() {
      ...
      const services = Layer.merge(
        Layer.succeed(SqlClient.SqlClient, blockingSql),
        Layer.succeed(ReplicaBootstrap.ReplicaBootstrap, bootstrap)
      )
      const context = yield* Layer.build(ReplicaGate.layer.pipe(Layer.provide(services)))
      const gate = Context.get(context, ReplicaGate.ReplicaGate)
      const exclusive = yield* Effect.scoped(gate.exclusive).pipe(Effect.forkChild)
      yield* Deferred.await(committed)
      const interruption = yield* Fiber.interrupt(exclusive).pipe(Effect.forkChild)
      yield* Effect.yieldNow
