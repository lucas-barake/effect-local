  ...
  yield* Effect.acquireRelease(
    ...
    () =>
      presenceGate.withPermit(
        sql`UPDATE effect_local_server_watch_runtimes SET expires_at = 0
    ...
    yield* Effect.acquireRelease(
      ...
      () =>
        presenceGate.withPermit(Effect.gen(function*() {
          ...
          const removed = yield* sql`DELETE FROM effect_local_server_watch_presence
            WHERE space_id = ${spaceId} AND client_id = ${clientId} AND watcher_id = ${watcherId}`.pipe(
            Effect.as(true),
            Effect.catchTag("SqlError", (cause) => {
              presenceDirty = true
              return Effect.logWarning("Offline wake presence cleanup failed", cause).pipe(Effect.as(false))
            }),
            Effect.tapDefect(() => {
              presenceDirty = true
              return Effect.void
          ...
          if (removed) {
            yield* sql`UPDATE effect_local_server_offline_wakes SET next_attempt_at = 0
              WHERE space_id = ${spaceId} AND client_id = ${clientId}`.pipe(
              Effect.catchTag("SqlError", (cause) => Effect.logWarning("Offline wake release prompt failed", cause))
            )
          }
          yield* notify
        }))
