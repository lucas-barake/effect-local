...
const batchedSyncRepliesMigration = Effect.gen(function*() {
  ...
  yield* sql`CREATE INDEX effect_local_migration_14_receipt_match
    ON effect_local_peer_receipts(
      replica_incarnation, peer_id, document_id, reply_hash, row_id
    ) WHERE reply IS NOT NULL AND reply_hash IS NOT NULL`
  yield* sql`CREATE INDEX effect_local_migration_14_outbox_match
    ON effect_local_peer_outbox(
      replica_incarnation, peer_id, document_id, message_hash,
      connection_epoch, send_sequence, status
    )`
  yield* sql`CREATE TEMP TABLE effect_local_migration_14_reply_matches (
    replica_incarnation INTEGER NOT NULL,
    peer_id TEXT NOT NULL,
    connection_epoch TEXT NOT NULL,
    send_sequence INTEGER NOT NULL,
  ...
  yield* sql`INSERT INTO effect_local_migration_14_reply_matches
  ...
  yield* sql`CREATE TEMP TABLE effect_local_migration_14_reply_coverage (
    reply_row_id INTEGER PRIMARY KEY,
    has_sent INTEGER NOT NULL
  )`
  yield* sql`INSERT INTO effect_local_migration_14_reply_coverage
  ...
  yield* sql`UPDATE effect_local_peer_receipt_replies AS reply
    SET status = 'Sent'
    WHERE EXISTS (
      SELECT 1 FROM effect_local_migration_14_reply_coverage AS coverage
      WHERE coverage.reply_row_id = reply.row_id
        AND (
          coverage.has_sent = 1 OR reply.row_id NOT IN (
            SELECT reply_row_id FROM effect_local_migration_14_reply_matches
  ...
  yield* sql`UPDATE effect_local_peer_outbox AS outbox
    SET receipt_reply_id = (
      ...
      FROM effect_local_migration_14_reply_matches AS match
    ...
    WHERE EXISTS (
      SELECT 1 FROM effect_local_migration_14_reply_matches AS match
      WHERE match.replica_incarnation = outbox.replica_incarnation
        AND match.peer_id = outbox.peer_id
        AND match.connection_epoch = outbox.connection_epoch
        AND match.send_sequence = outbox.send_sequence
    )`
  yield* sql`DROP TABLE effect_local_migration_14_reply_coverage`
  yield* sql`DROP TABLE effect_local_migration_14_reply_matches`
  yield* sql`DROP INDEX effect_local_migration_14_outbox_match`
  yield* sql`DROP INDEX effect_local_migration_14_receipt_match`
  ...
  yield* sql`INSERT INTO effect_local_migration_catalog (migration_id, name, checksum)
    VALUES (14, 'batched_sync_replies', ${batchedSyncRepliesChecksum})`
})
...
export const loader = Migrator.fromRecord({
  ...
  "14_batched_sync_replies": batchedSyncRepliesMigration
})
