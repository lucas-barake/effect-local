...
export default defineConfig({
  ...
  use: {
    ...
    launchOptions: {
      // Each context is a separate device. Chromium otherwise suspends the hidden context's
      // dedicated database worker while Playwright interacts with the other device.
      args: [
  ...
  webServer: [
    ...
    {
      command: `pnpm build && pnpm exec vite preview --host 127.0.0.1 --port ${appPort} --strictPort`,
      cwd,
