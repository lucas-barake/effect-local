...
| **Command outcome**          | What a `lookup*` answers: `DurablyCommittedLocal`, a declared `Rejected` result, or `OutcomeUnknown`. Commands themselves return the committed value and report the other two through the Effect error channel, alongside `ReplicaError`.     |
...
`Replica` is an Effect service. `create`, `mutate` and `delete` return the committed value and put every failure in
the error channel, the same way the rest of the library reads.
...
const rename = (documentId: Identity.DocumentId) =>
  Effect.gen(function*() {
    ...
    // The mutation's own declared error arrives unwrapped, so it is caught by its own tag.
    Effect.catchTag("RenameError", (error) => Effect.succeed({ _tag: "Rejected" as const, error })),
    // Ambiguity is never retryable: the same command id has to be looked up, not reissued.
    Effect.catchReason("CommandOutcomeUnknown", ({ commandId }) =>
      Effect.succeed({ _tag: "RetryLookup" as const, commandId }))
  )
```

How a command can end:

| Result                                   | Meaning                                                                    | Application action                                      |
| ---------------------------------------- | -------------------------------------------------------------------------- | ------------------------------------------------------- |
| Success                                  | Canonical state, projections, receipt, and Cluster reply committed locally | Use the returned value                                  |
| The mutation's declared error            | The mutation handler rejected the command with its declared domain error   | Show or handle the domain error                         |
| `ReplicaError` / `CommandOutcomeUnknown` | The caller cannot prove whether the command committed                      | Keep the command ID and call the matching lookup method |

The `lookup*` methods still answer with a `CommandOutcome` value, because "what happened to this id" genuinely has
three answers. `CommandOutcome.committedOrFail` projects one into the same channels a command uses.
...
| Method                                                 | Result                                                                                        |
| ------------------------------------------------------ | --------------------------------------------------------------------------------------------- |
| `create(document, { commandId, value })`               | `DocumentId`                                                                                  |
| `get(document, documentId)`                            | Decoded `Snapshot` with heads, version, tombstone, and projection state                       |
| `mutate(mutation, { commandId, documentId, payload })` | The mutation's success value                                                                  |
| `delete(document, { commandId, documentId })`          | `void` and a durable tombstone                                                                |
...
`committedOrFail` projects a `lookup*` outcome the way a command already reports itself: the committed value on
success, the declared mutation error unwrapped for `Rejected`, and `ReplicaError` with a `CommandOutcomeUnknown`
reason when durability cannot be established. Every failure path is tagged and works with `Effect.catchTag` and
`Effect.catchReason`.
...
The browser restore protocol begins with a short unary request. The owner returns a restore nonce and dedicated
`MessagePort`. The page immediately starts a pending `FinishRestoreBackup` result RPC and sends `Start`. After the
...
it.layer(NodeCrypto.layer)("replica", (it) => {
  it.effect("commits locally", () =>
    Effect.gen(function*() {
      const replica = yield* Replica.Replica
      const now = Date.now()
      // No unwrapping: `create` fails unless the command is durably committed.
      yield* replica.create(Task, {
        commandId: yield* Identity.makeCommandId,
...
| `CommandOutcome`    | `Rejected`, `DurablyCommittedLocal`, `OutcomeUnknown`, `CommandOutcome`, `schema`, `rejected`, `durablyCommitted`, `unknown`, `match`, `committedOrFail`, `toOutcome`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `ReplicaError`      | Reason schemas `DocumentNotFound`, `DocumentDecodeError`, `DocumentEncodeError`, `UnsupportedDocumentVersion`, `ProjectionBlocked`, `CommandIdConflict`, `ReceiptOperationMismatch`, `StorageUnavailable`, `CanonicalEncodeError`, `StorageCorrupt`, `ReplicaMetadataMissing`, `QuotaExceeded`, `MigrationFailed`, `BackupInvalid`, `BackupTooLarge`, `RestoreBusy`, `RestoreFailed`, `ProtocolMismatch`, `ReplicaFenced`, `OperationTimeout`, `UnsupportedStorageFormatVersion`, `CheckpointSuperseded`, `DocumentLineageChanged`, `CommandOutcomeUnknown`. Causal reasons use `Schema.Defect()` for transportable arbitrary failures. `Reason`, tagged `ReplicaError`, `isReplicaError` |
...
Commands return their committed value and fail with the declared error or `ReplicaError`, as query does. Status and backup
...
| `ReplicaMetadataMissing`                                                                         | `operation`                                         |
...
| `PeerRelayClientRuntime` | `ConnectionConfiguration`, `PeerRelayClientRuntime` service, `makeScoped`, `layer`, `layerSql`                                                                                                                                                                                                                            |
...
| `SqlReplica`             | `layerFromServices`, `layer`, `layerRelay`, `layerWithBindings`, `layerRelayWithBindings`                                                                                                                                                                                                                                 |
...
| `BeginRestoreBackup`  | `mode`, `maxBytes`, `expectedDefinitionHash`, `installationId` | `{ nonce, port }`. The page sends Start, then the owner drives Pull, chunk, terminal readiness, acknowledgement, and release confirmation |
| `FinishRestoreBackup` | `nonce`                                                        | Pending authoritative restore result. The page accepts its Exit before acknowledging `TerminalReady`                                      |
