...
- The duplication the rule above accepts is inline construction at the call site, not a repeated helper definition. A module local error constructor is fine when one module raises the same error several times. Do not copy that constructor into a second module. Once more than one module needs it, move it to a single module under `src/internal` and import it there. Two modules that build the same error two different ways are two representations of one concept, and a reader who meets the second one cannot tell whether it matches the first without diffing them.
